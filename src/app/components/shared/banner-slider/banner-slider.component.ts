  ...
  @for(banner of banners(); track banner.id; let i = $index) {
    <div 
      ...
      <div class="absolute bottom-0 left-0 p-8 md:p-12 text-white">
        ...
        <button routerLink="/courses" class="mt-6 px-6 py-3 bg-indigo-600 hover:bg-indigo-700 text-white font-bold rounded-lg shadow-lg transition-transform transform hover:scale-105">
          {{ banner.ctaText }}
        </button>
      </div>
    </div>
...
export class BannerSliderComponent implements onInit, OnDestroy {
  ...
  banners = this.courseService.allBanners;
